...
INITIAL_STALENESS_THRESHOLD = 35.0
...
async def client_task(client_id: int, trainloader: DataLoader, global_model_ref: list, global_version_ref: list, update_queue: asyncio.Queue):
    for round_num in range(1, NUM_ROUNDS_PER_CLIENT + 1):
        ...
        delay = random.uniform(8, 12) if client_id in [0, 1, 2] else random.uniform(1.5, 3.5)
...
def calculate_dynamic_alpha(staleness: int, threshold: float) -> float:
    if staleness >= threshold:
        return 0.0
    decay_factor = staleness / threshold
...
async def server_task(global_model_ref: list, global_version_ref: list, testloader: DataLoader, update_queue: asyncio.Queue, total_updates: int, results_log: list, simulation_start_time: float):
    ...
    staleness_history = deque(maxlen=ADAPTIVE_WINDOW_SIZE)
    dynamic_threshold = INITIAL_STALENESS_THRESHOLD
    ...
    for updates_processed in range(1, total_updates + 1):
        ...
        if len(staleness_history) > ADAPTIVE_WINDOW_SIZE / 2:
            sorted_history = sorted(list(staleness_history))
            # Consider the fastest 75% of recent clients
            fast_clients_staleness = sorted_history[:int(len(sorted_history) * 0.75)]
            avg_fast_staleness = np.mean(fast_clients_staleness)
            dynamic_threshold = max(10.0, avg_fast_staleness * ADAPTIVE_THRESHOLD_MULTIPLIER)

        dynamic_alpha = calculate_dynamic_alpha(staleness, dynamic_threshold)
        
        print(f"\n[Server] Received update #{updates_processed}/{total_updates}. Staleness: {staleness}. Dynamic Threshold: {dynamic_threshold:.1f}. Dynamic Alpha: {dynamic_alpha:.3f}")
...
async def main():
    print("Starting asynchronous FL with adaptive staleness threshold simulation...")
    ...
    print("\n Asynchronous FL with adaptive staleness threshold simulation finished")
    
    with open("async_adaptive_staleness_results.json", "w") as f:
        json.dump(results_log, f, indent=2)
    print("Adaptive staleness results saved to 'async_adaptive_staleness_results.json'")
